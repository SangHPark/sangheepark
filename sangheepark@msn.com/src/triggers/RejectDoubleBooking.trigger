trigger RejectDoubleBooking on Session_Speaker__c (before insert, before update) {
    ...
    List<Session__c> related_sessions = 
        [SELECT ID, Session_Date__c from session__c WHERE ID IN :requested_bookings.keySet()];
    for(Session__c related_session : related_sessions) {
        requested_bookings.put(related_session.Id, related_session.Session_Date__c);
    }
    
    List<Session_Speaker__c> related_speakers = 
